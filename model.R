...
model <- function(
	...
	# Collapse raw reads on suitable phyla level, filter, inverse normal rank transformation, and pick correct columsn for test
	# genus
	...
	output_final[,"Score"] <- 
		output_temp[,"module_agesex"] + 
		output_temp[,"module_metamix"] +
		output_temp[,"module_genus_glmnet"] +
		output_temp[,"module_family_glmnet"]
		
	# Scale within [0,1] as instructed
	output_final[,"Score"] <- outscale(output_final[,"Score"])
	# Take in modules for sanity checking
	#output_final[,"module_agesex"] <- output_temp[,"module_agesex"]
	#output_final[,"module_metamix"] <- output_temp[,"module_metamix"]
	#output_final[,"module_genus_glmnet"] <- output_temp[,"module_genus_glmnet"]
	#output_final[,"module_family_glmnet"] <- output_temp[,"module_family_glmnet"]
